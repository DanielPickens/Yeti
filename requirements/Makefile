...
PKG := github.com/danielpickens/yeti
...
DOCKER_REGISTRY := quay.io/danielpickens

BUILDER_IMG := $(DOCKER_REGISTRY)/yeti-builder:1.0
UI_BUILDER_IMG := $(DOCKER_REGISTRY)/yeti-ui-builder:1.0
yeti_IMG := $(DOCKER_REGISTRY)/yeti:$(GIT_COMMIT)
...
build-s3-client-image:
	docker build -t quay.io/danielpickens/s3-client:0.0.1 -f Dockerfile-s3-client .
	docker push quay.io/danielpickens/s3-client:0.0.1
...
build-image: ## Build yeti image
	docker build -t $(yeti_IMG) .
	docker push $(yeti_IMG)

tag-release: ## Tag yeti image as release
	docker tag $(yeti_IMG) $(DOCKER_REGISTRY)/yeti:$(VERSION)
	docker push $(DOCKER_REGISTRY)/yeti:$(VERSION)
...
yeti-dev: ## Run yeti(be and fe) in development mode
...
stop-dev:
	telepresence leave yeti-yeti-system
