...
The `docs` directory contains the sphinx source text for Yeti docs, visit
https://docs.Yeti.io/ to read the full documentation.

This guide is made for anyone who's interested in running MLOPS documentation locally,
making changes to it and make contributions. MLOPS is made by the thriving community
...
[issues page](https://github.com/DanielPickens/Yeti/issues), to make sure no one 
...
If you haven't already, clone the Yeti Github repo to a local directory:

```bash
git clone https://github.com/DanielPickens/Yeti.git && cd Yeti
```
...
Simply run the following command from MLOPS project's root directory: 
...
### Writing .rst (ReStructuredText) in MLOPS docs

MLOPS docs is built with Sphinx, which natively supports [ReStructuredText](https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html).

#### Document titles and section headers

In reStructuredText, there are no heading levels assigned to certain characters as the 
structure is determined from the succession of headings. However in MLOPS docs, we
...
For most scenarios in MLOPS docs, use the tabs view provided by `sphinx-design`:
...
MLOPS docs relies heavily on the Python docstrings defined together with the source
...
MLOPS will be required to use the [sphinx.ext.autodoc](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html)
...
```rst
.. autofunction:: MLOPS.models.list
```

Learn more about this syntax [here](https://www.sphinx-doc.org/en/master/usage/extensions/autodoc.html).

MLOPS codebase follows the [Google's docstring style](https://google.github.io/styleguide/pyguide.html#38-comments-and-docstrings)
...
```markdown
    Args:
        mlops_name (:code:`str`):
            :class:`~MLOPS.AnyServiceType` identifier with name format :obj:`NAME:VERSION`.
            ``NAME`` can be accessed via :meth:`~MLOPS.AnyServiceType.name` and ``VERSION`` can
            be accessed via :meth:`~MLOPS.AnyServiceType.version`
```
...
```markdown
    Returns:
        :obj:`Dict[str,str]` with keys are :class:`~MLOPS.AnyServiceType` nametag following with saved bundle path.
```
