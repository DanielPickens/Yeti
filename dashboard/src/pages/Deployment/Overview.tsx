...
export default function DeploymentOverview() {
    ...
    let imageBuilderPodNamespace = 'yeti-builders'
    let imageBuilderPodSelector = `yeti.ai/ml-repository=${deployment?.latest_revision?.targets[0]?.ml.repository.name},yeti.ai/ml=${deployment?.latest_revision?.targets[0]?.ml.version}`
    if (
        _.startsWith(
            yetiComponentsInfo?.data?.find((component) => component.name === 'deployment')?.manifest
                ?.latest_crd_version ?? 'v1alpha2',
            'v2'
        )
    ) {
        imageBuilderPodNamespace = kubeNamespace
        imageBuilderPodSelector = `yeti.ai/is-ml-image-builder=true,yeti.ai/ml-repository=${deployment?.latest_revision?.targets[0]?.ml.repository.name},yeti.ai/ml=${deployment?.latest_revision?.targets[0]?.ml.version}`
    }
                    ...
                    data={[
                        [
                            ...
                            deployment?.latest_revision && deployment.latest_revision.targets.length > 0 && (
                                <Link
                                    key={deployment.uid}
                                    href={`/ml_repositories/${deployment.latest_revision.targets[0].ml.repository.name}/mls/${deployment.latest_revision.targets[0].ml.version}`}
                                >
                                    {deployment.latest_revision.targets[0].ml.repository.name}:
                                    {deployment.latest_revision.targets[0].ml.version}
                                </Link>
                            ),
