...
import { useFetchmlOptional } from '@/hooks/useFetchml'
...
export default function LokiLog({ deployment: deployment_, podName, namespace, style }: ILokiLogProps) {
    ...
    const mlInfo = useFetchmlOptional(
                ...
                >
                    ...
                    >
                        ...
                        <Select
                            ...
                            options={[
                                {
                                    id: 'yeti_ai_is_ml_api_server="true"',
                                    label: (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                            {React.createElement(resourceIconMapping.ml_api_server, { size: 12 })}
                                            <span>API Server</span>
                                        </div>
                                    ),
                                    filterNode: {
                                        name: 'yeti_ai_is_ml_api_server',
                                        value: 'true',
                                        operator: '=',
                                    },
                                },
                                ...(mlInfo?.data?.manifest?.runners?.map((runner) => {
                                    return {
                                        id: `yeti_ai_ml_runner="${runner.name}"`,
                                        label: (
                                            <div
                                                style={{
                                                    display: 'flex',
                                            ...
                                            >
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                    {React.createElement(resourceIconMapping.ml_runner, {
                                                        size: 12,
                                                    })}
