...
import { mlmlConfigsEnvKey, resourceIconMapping, sidebarExpandedWidth, sidebarFoldedWidth } from '@/consts'
...
import mlRepositorySelector from './mlRepositorySelector'
...
export default function DeploymentForm({
    ...
    const [ml, setml] = useState<ImlWithRepositorySchema>()
    ...
    const [mlmlConfs, setmlmlConfs] = useState<string[]>([])
    ...
    useEffect(() => {
        ...
        setmlmlConfs(
            deploymentRevision.targets.map(
                (target) => target.config?.envs?.find((env) => env.key === mlmlConfigsEnvKey)?.value ?? ''
            )
        )
        const values0 = {
            ...
            targets: deploymentRevision.targets.map((target) => {
                return {
                    type: target.type,
                    ml_repository: target.ml.repository.name,
                    ml: target.ml.version,
    ...
    const handleFinish = useCallback(async () => {
        ...
        values.targets.forEach((target, idx) => {
            ...
            const confItem = {
                key: mlmlConfigsEnvKey,
                value: mlmlConfs[idx] ?? '',
            }
            if (confItem.value) {
                const configsIdx = target.config.envs.findIndex((env) => env.key === mlmlConfigsEnvKey)
                if (configsIdx >= 0) {
                    // eslint-disable-next-line no-param-reassign
                ...
                // eslint-disable-next-line no-param-reassign
                target.config.envs = target.config.envs.filter((env) => env.key !== mlmlConfigsEnvKey)
            }
            ...
            Object.keys(target.config.runners).forEach((key) => {
                ...
                if (confItem.value) {
                    const configsIdx_ = runner.envs.findIndex((env) => env.key === mlmlConfigsEnvKey)
                    if (configsIdx_ >= 0) {
                        // eslint-disable-next-line no-param-reassign
                    ...
                    // eslint-disable-next-line no-param-reassign
                    runner.envs = runner.envs.filter((env) => env.key !== mlmlConfigsEnvKey)
                }
            })
        })
        setLoading(true)
    ...
    }, [mlmlConfs, history, onSubmit, values])
    ...
    useEffect(() => {
        setUIValues((vs) => {
            ...
            const values_ = {
                ...vs,
                kube_namespace: clusterInfo.data.config?.default_deployment_kube_namespace ?? 'yeti',
            }
            return values_
        })
    }, [clusterInfo.data])
    ...
    useEffect(() => {
        setRunnerTabsActiveKey((runnerTabsActiveKey_) => {
            if (
                ml?.manifest?.runners &&
                ml.manifest.runners.length > 0 &&
                !ml.manifest.runners.find((runner) => runner.name === runnerTabsActiveKey_)
            ) {
                return ml.manifest.runners[0].name
            }
                ...
                targets: [
                    {
                        ...
                        config: {
                            ...vs.targets[0].config,
                            runners:
                                ml?.manifest?.runners?.reduce((runners, runner) => {
                                    const conf = {
                                        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    ...
    }, [ml?.manifest?.runners, deploymentRevision])
                                ...
                                {uiValues.targets.map((target, idx) => {
                                    return (
                                        <div key={idx}>
                                            ...
                                            <Divider orientation='left'>{t('select ml')}</Divider>
                                            <div>
                                                <FormItem
                                                    style={{ display: 'none' }}
                                                    required
                                                ...
                                                >
                                                    <FormItem
                                                        required
                                                        name={['targets', idx, 'ml_repository']}
                                                        ...
                                                        label={
                                                            ...
                                                            >
                                                                {React.createElement(
                                                                    resourceIconMapping.ml_repository,
                                                                    {}
                                                                )}
                                                                <div>{t('ml repository')}</div>
                                                            </div>
                                                        }
                                                    >
                                                        <mlRepositorySelector />
                                                    </FormItem>
                                                    {target.ml_repository && (
                                                        <FormItem
                                                            required
                                                            name={['targets', idx, 'ml']}
                                                            label={
                                                                ...
                                                                >
                                                                    {React.createElement(resourceIconMapping.ml, {})}
                                                                    <div>{t('ml')}</div>
                                                                </div>
                                                            }
                                                            style={{ width: 370 }}
                                                        >
                                                            <mlSelector
                                                                mlRepositoryName={target.ml_repository}
                                                                onmlChange={setml}
                                                            />
                                                        </FormItem>
                                                    )}
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <VscServerProcess />
                                                        <LabelSmall>{t('mlml configuration')}</LabelSmall>
                                                    </div>
                                                    <div
                                                        style={{
                                                    ...
                                                    >
                                                        <MonacoEditor
                                                            ...
                                                            onChange={(value) => {
                                                                setmlmlConfs((mlmlConfs_) => {
                                                                    const newmlmlConfs = mlmlConfs_.slice()
                                                                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                                                                    newmlmlConfs[idx] = value ?? ''
                                                                    return newmlmlConfs
                                                                })
                                                            }}
                                                        />
                                                ...
                                                >
                                                    <Panel title='Runners'>
                                                        ...
                                                        >
                                                            {ml?.manifest.runners?.map((runner) => (
                                                                <Tab
                                                                    key={runner.name}
                                                                    title={
                                                                        <div
